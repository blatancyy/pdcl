exports.run = async(client, message, args) => {
	...
	const [rows, fields] = await db.execute(`SELECT * FROM mute_data WHERE league_id = "${message.guild.id}" AND target_id = "${args[0]}" AND has_expired = 0 AND staff_id = "${message.author.id}";`);
